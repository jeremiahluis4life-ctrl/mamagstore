 ...
 * We normalize the data structure so the rest of the script works 
 * whether it's a Webhook or a Fetch call.
 */
$isWebhook = isset($_SERVER['HTTP_X_PAYSTACK_SIGNATURE']);
...
if ($isWebhook) {
    ...
    $processed_data = [
        ...
        'amount'        => (float)($raw_data['data']['amount'] / 100), // Paystack is in kobo
        'total_paid'    => (float)($raw_data['data']['amount'] / 100),
        'items'         => json_decode($metaMap['order_details_json'] ?? '[]', true), // Ensure you send JSON in JS metadata
        'delivery_fee'  => 0, // Metadata would need this passed explicitly
...
/**
 * 2. IDEMPOTENCY CHECK
 * Prevent processing the same order twice if Webhook and Fetch hit at the same time.
 */
$checkDuplicate = mysqli_query($conn, "SELECT id FROM mamag_orders WHERE reference = '$reference' AND status = 'Paid'");
...
/**
 * 3. VERIFY PAYMENT STATUS WITH PAYSTACK
 */
    ...
    try {
        ...
        if (!empty($processed_data['items']) && is_array($processed_data['items'])) {
            foreach ($processed_data['items'] as $item) {
                ...
                if ($isSpecial) {
                    $clean_id = (int)$sent_id;
                    mysqli_query($conn, "UPDATE mamag_requests SET status = 'Purchased' WHERE id = $clean_id");
                } 
                else if ($isAddon) {
                    mysqli_query($conn, "UPDATE mamag_addons SET stock = GREATEST(0, stock - $qty) WHERE id = '$sent_id'");
                } 
                else {
                    // Check if it's a sub-item or main product
